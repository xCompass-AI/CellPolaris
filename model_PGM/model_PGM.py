...
def data_pro1(Network_file,RNA_file,edge_num,name):
    ...
    network['Score'] = ((network['Score'] - min_val) / (max_val - min_val)) * 0.6 + 0.01
...
class Gauss_condition(nn.Module):
    ...
    def forward(self, x,y):
        if self.TF in TF_high:
            ...
            loc = (gTG_mu + self.k * self.cov * (y - gTF_high_mu)
                   / torch.square(gTF_high_sigma)).relu()+0.01
            scale = torch.sqrt((torch.square(gTG_sigma) - torch.square(self.alpha*self.cov)
            ...
            loc = (gTG_mu + self.k * self.cov * (y - gTF_mu)
                   / torch.square(gTF_sigma)).relu() + 0.01
            scale = torch.sqrt((torch.square(gTG_sigma) - torch.square(self.alpha*self.cov)
                                 / torch.square(gTF_sigma)+1).relu()+0.01)
...
class Model(nn.Module):
    ...
    def forward(self,TF_high_exp,TG_exp,edge):
        ...
        for i in range(len(alpha_list)):
            g_res=torch.abs(torch.log(torch.abs(k_list[i] / alpha_list[i]) + 1e-10)/torch.log(torch.tensor(10.0)))
            g=g+g_res
...
def get_param_numpy(list_all, param_list, TG, TF_list, len_tf_tg,ko_tf):
    ...
    weight_value = list_all[index_k][3]
    tf_u = param_list[0][2 * index_tf]
    tf_sigma = param_list[0][2 * index_tf + 1]
    k = param_list[0][index_k + int(len_tf_tg)]
    weight_value= weight_value
    delta_x = -k * weight_value * tf_u / (tf_sigma ** 2*torch.sqrt(torch.abs(weight_value))+5)
    # delta_x = k
    print(delta_x)
    return delta_x
...
if __name__ == "__main__":
    ...
    data = pd.read_csv('./data_sample/RS1o2_sample.csv')
