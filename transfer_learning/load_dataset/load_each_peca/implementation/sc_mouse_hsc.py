...
class SingleCellMouseHSCPECA:
    ...
    def get_peca_files(self):
        trs_folder = self.root / "dataset/sc_mouse_hsc"
        trs_files = {}
        for fp in trs_folder.iterdir():
            if "network" in fp.stem:
                trs_files[fp.stem.split("_")[0]] = fp
        rna_seq_folder = self.root / "dataset/sc_mouse_hsc"
        rna_seq_files = {}
    ...
    def load_raw_peca(self):
        ...
        for name, trs_file in self.trs_files.items():
            ...
            raw_peca_data[name] = RawSingleCellHSC(trs_file, rna_seq_file)
...
if __name__ == "__main__":
    processor = SingleCellMouseHSCPECA()
    ...
