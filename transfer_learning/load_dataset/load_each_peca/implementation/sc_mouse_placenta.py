...
class SingleCellMousePlacentaPECA:
    def __init__(self):
    ...
    def get_peca_files(self):
        trs_folder = self.root / "dataset/sc_mouse_placenta"
        trs_files = {}
        for fp in trs_folder.iterdir():
            if "network" in fp.stem:
                trs_files["_".join(fp.stem.split("_")[:-1])] = fp
        rna_seq_folder = self.root / "dataset/sc_mouse_placenta"
        rna_seq_files = {}
    ...
    def load_raw_peca(self):
        ...
        for name, trs_file in self.trs_files.items():
            ...
            raw_peca_data[name] = RawSingleCellPlacenta(trs_file, rna_seq_file)
...
if __name__ == "__main__":
    processor = SingleCellMousePlacentaPECA()
    ...
