...
from .tr_loss.extra_mixup import Extra_mixup
...
class TransferLoss(nn.Module):
    def __init__(
        ...
        elif self.loss_type in ["graph_extramixup"]:
            self.mixup_alpha = mixup_alpha
            self.mixup_ex = Extra_mixup(alpha=self.mixup_alpha)
        elif self.loss_type in ["graph_twokindmixup"]:
            self.mixup_alpha = mixup_alpha
            self.mixup = Mixup(alpha=self.mixup_alpha)
            self.mixup_ex = Extra_mixup(alpha=self.mixup_alpha)
    ...
    def compute(self, x0, x1, **kwargs):
        ...
        Arguments:
            ...
            kwargs {dict} -- other arguments, containing:
                ...
                For graph_mixup and graph_extramixup:
                    y {tensor} -- edge weight (label)
                    link_prediction_model {nn.Module} -- link prediction model
                    embed0 {tensor} -- embedding matrix of tfs
                    embed1 {tensor} -- embedding matrix of tgs
        ...
        elif self.loss_type == "graph_extramixup":
            loss = self.mixup_ex(
                link_prediction_model=kwargs["link_prediction_model"],
                x0=x0,
                x1=x1,
                ...
                embed1=kwargs["embed1"],
                y=kwargs["y"],
                strong_related_source=kwargs["strong_related_source"], 
                tissue_index_map=kwargs["tissue_index_map"],
            )
        elif self.loss_type == "graph_twokindmixup":
            loss_ex = self.mixup_ex(
                link_prediction_model=kwargs["link_prediction_model"],
                x0=x0,
                x1=x1,
                embed0=kwargs["embed0"],
