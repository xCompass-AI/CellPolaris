...
class TRModel(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ignore_regression (bool): whether to ignore regression loss
                ...
                while graph_mmd will not, this will influence whether to freeze bn layers.
        """
        ...
        self.strong_related_source = strong_related_source
        self.tissue_index_map = tissue_index_map
        if loss_type is not None:
            self.transfer_loss = TransferLoss(
                ...
                strong_related_source = self.strong_related_source,
                tissue_index_map = self.tissue_index_map,
            )
    ...
    def forward(
        ...
        if self.transfer_loss_type is None:
            ...
            if self.transfer_loss_type == "graph_twokindmixup":
                in_species_list = []
                for tissue_data in self.tissue_index_map.values():
                    in_species_list.append(tissue_data['mouse'])
                    in_species_list.append(tissue_data['human'])

                feature = tuple(f for i, f in enumerate(feature) if i not in in_species_list)
            ...
            elif mode == "extramixup": #graph_extramixup
                embed0 = torch.split(embed0, batch_sizes)
                embed1 = torch.split(embed1, batch_sizes)
                loss = self.transfer_loss.compute(
                    x0=x0,
