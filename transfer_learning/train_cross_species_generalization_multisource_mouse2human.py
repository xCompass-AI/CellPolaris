...
def merge_train_test(train_loaders, test_loaders, device):
    ...
    if train_loaders is not None and test_loaders is not None:
        all_loaders = list(train_loaders) + list(test_loaders)
    elif train_loaders is not None:
        all_loaders = train_loaders
    elif test_loaders is not None:
        all_loaders = test_loaders
    else:
        raise ValueError("None dataloader")
    all_x0, all_x1, all_edges, all_y = [], [], [], []
    for loader in all_loaders:
        for batch in loader:
            if len(batch) == 4:
                x0, x1, edge, y = batch
            elif len(batch) == 5:
                x0, x1, edge, y, _ = batch
            else:
    ...
    x0_all = torch.cat(all_x0, dim=0).to(device)
    x1_all = torch.cat(all_x1, dim=0).to(device)
    edge_all = torch.cat(all_edges, dim=0).to(device)
    y_all = torch.cat(all_y, dim=0).to(device)

    return x0_all, x1_all, edge_all, y_all

def save_edges_to_txt(edges, filename):
    with open(filename, 'w') as f:
        for e in edges.cpu().tolist():
            f.write(f"{e}\n")

def get_target_specific(target_data, source_data):

    x0_t, x1_t, edge_t, y_t = target_data
    _, _, edge_s, _ = source_data

    edge_t_set = set(map(tuple, edge_t.cpu().tolist()))
    edge_s_set = set(map(tuple, edge_s.cpu().tolist()))

    specific_edge_set = edge_t_set - edge_s_set

    index_specific = [
        i for i, e in enumerate(map(tuple, edge_t.cpu().tolist())) if e in specific_edge_set
    ]
    index_specific = torch.tensor(index_specific, dtype=torch.long, device=edge_t.device)

    x0_spec = x0_t[index_specific]
    x1_spec = x1_t[index_specific]
    edge_spec = edge_t[index_specific]
    y_spec = y_t[index_specific]

    return x0_spec, x1_spec, edge_spec, y_spec
...
if __name__ == "__main__":
    ...
    args.add_argument("--tissue_list", nargs="+", type=str, default=["spleen", "liver", "kidney", "lung","heart"], help="source tissues from human and mouse")
    args.add_argument("--target",type=str, default="heart", help="this is from human bulk, for cross species")
    args.add_argument(
        "--append_bulk",
        action="store_true",
        default=False,
    ...
    args.add_argument("--fold", "--k-of-4-fold", type=int, default=1)
    args.add_argument(
        "--dg_loss_type",
        type=lambda x: None if x == "None" else x,
        default="graph_extramixup", #graph_mixup "graph_extramixup"
    )
    ...
    args.add_argument("--top_ratio", type=float, default=0.11, help="if dg_loss_type is graph-based, this is a hyper-parameter")
    ...
    args.add_argument("--analysis_train_test_set",type=str,default='test',help ="analyse both'train&test' or only 'test'set")
    args.add_argument(
        "--specific",
        action="store_true",
        default=False,
        help="Test the specific edges of target.",
    )
    args.add_argument(
        "--test",
        action="store_true",
    ...
    if len(config.species)>1:
        if "bulk_mouse" in train_loaders.keys():
            ...
            select_tissue = ["spleen", "liver", "heart", "kidney", "lung"]
            for tissue_name, tissue in train_loaders["bulk_mouse"].items():
                if tissue_name in select_tissue:
                    ...
                    all_train_loaders["bulk_mouse"].append((tissue_name, loader))
                    all_test_loaders["bulk_mouse"].append(
                        ((tissue_name, test_loaders["bulk_mouse"][tissue_name][period_name]))
                    )
        if "bulk_human" in train_loaders.keys():
            all_train_loaders["bulk_human"], all_test_loaders["bulk_human"] = [], []
            select_tissue = ["spleen", "right_lobe_of_liver", "Fetal_Heart", "Fetal_Kidney", "Fetal_Lung"]
            for name, loader in train_loaders["bulk_human"].items():
                if name in select_tissue:
                    all_train_loaders["bulk_human"].append((name, loader))
                    all_test_loaders["bulk_human"].append((name, test_loaders["bulk_human"][name]))
        all_train_loaders["bulk_mouse"] = { t[0]: t[1] for t in all_train_loaders["bulk_mouse"] }
        all_test_loaders["bulk_mouse"] = { t[0]: t[1] for t in all_test_loaders["bulk_mouse"] }
        all_train_loaders["bulk_human"] = { t[0]: t[1] for t in all_train_loaders["bulk_human"] }
        all_test_loaders["bulk_human"] = { t[0]: t[1] for t in all_test_loaders["bulk_human"] }
    ...
    if all(x in ["bulk_mouse", "bulk_human", "sc_mouse", "sc_human"] for x in species):
        if len(species)>1:
            if species == ["sc_mouse", "sc_human"]:
                ...
                for key, loader in all_train_loaders["bulk_human"].items():
                    if target in key.lower():
                        target_train_loaders.append(all_train_loaders["bulk_human"][key])
                        target_test_loaders.append(all_test_loaders["bulk_human"][key])
                        break
                        ...
                        for key, loader in all_train_loaders["bulk_human"].items():
                            ...
                            for tissue in sources:
                                ...
                                if tissue_lower in key_lower:
                                    ...
                                    source_train_loaders.append(loader)
                                    source_test_loaders.append(all_test_loaders["bulk_human"][key])
                                    species_mask.append("human")

                                    if tissue_index_map[tissue_lower]["human"] is None:
                                        tissue_index_map[tissue_lower]["human"] = len(source_train_loaders) - 1
                                    break

                    elif spe == "bulk_mouse":
                        for key, loader in all_train_loaders["bulk_mouse"].items():
                            key_lower = key.lower()
                            for tissue in config.tissue_list:
                                tissue_lower = tissue.lower()
                                if tissue_lower in key_lower:
                                    source_train_loaders.append(loader)
                                    source_test_loaders.append(all_test_loaders["bulk_mouse"][key])
                                    species_mask.append("mouse")

                                    if tissue_lower == target:
                                        strong_related_source = len(source_train_loaders) - 1

                                    elif tissue_lower in tissue_index_map and tissue_index_map[tissue_lower]["mouse"] is None:
                                        tissue_index_map[tissue_lower]["mouse"] = len(source_train_loaders) - 1
                                    break
    ...
    human_train_loaders, human_test_loaders = [], []
    if config.analysis_train_test_set=='train&test':
        target_data_all = merge_train_test(target_train_loaders, target_test_loaders, config.device)
        human_train_loaders = [loader for loader, species in zip(source_train_loaders, species_mask)if species == "human"]
        human_test_loaders = [loader for loader, species in zip(source_test_loaders, species_mask)if species == "human"]
        source_data_all = merge_train_test(human_train_loaders, human_test_loaders, config.device)
    elif config.analysis_train_test_set=='test':
        target_data_all = merge_train_test(train_loaders=None, test_loaders=target_test_loaders, device=config.device)
        human_train_loaders = [loader for loader, species in zip(source_train_loaders, species_mask)if species == "human"]
        source_data_all = merge_train_test(human_train_loaders, test_loaders=None, device=config.device)
    x0_spec, x1_spec, edge_spec, y_spec = get_target_specific(target_data_all, source_data_all)
    target_specific_dataset = TensorDataset(x0_spec, x1_spec, edge_spec, y_spec)
    target_specific_loader = DataLoader(target_specific_dataset, batch_size=config.batch_size, shuffle=False)
    ...
    if config.dg_loss_type == 'graph_mixup':
        file_name = (
            ...
            f"{config.species}/mixupalpha{config.mixup_alpha}_topratio{config.top_ratio}/target_{target}"
        )
    else:
        file_name = (
            base / f"{config.model}/{config.dg_loss_type}/"
    ...
    if config.test:
        if config.specific:
            ...
            target_test_loss, target_test_pred, target_test_true = test(target_specific_loader)
            best_target_test_pred, best_target_test_true = target_test_pred, target_test_true
            ...
            target_test_loss, target_test_pred, target_test_true = test(target_test_loaders)
            best_target_test_pred, best_target_test_true = target_test_pred, target_test_true
