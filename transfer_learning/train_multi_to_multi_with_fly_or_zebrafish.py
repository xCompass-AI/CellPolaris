...
import copy
from map_3_species import load_third_species_link, to_human_mapping, map_human_edges_2_3spe
import pandas as pd
warnings.filterwarnings("error")

def get_3th_species(spe):
    if spe == 'fly':
        edges_3spe = pd.read_csv(Path(__file__).parent / "dataset/third_species/edges_fly_index.txt", sep="\t")
    elif spe == 'zebrafish':
        edges_3spe = pd.read_csv(Path(__file__).parent / "dataset/third_species/edges_zebrafish_index.txt", sep="\t")
    elif spe == None:
        edges_3spe = None
        print('error')
    return edges_3spe
...
def train(loader_iters, n_iter=10, dg_loss_weight=1, device="cpu", edges_3spe=None):
    model.train()
    ...
    for _ in range(n_iter):
        ...
        if edges_3spe is not None:
            edges = torch.cat(edges_list, dim=0)
            is_in_3spe = torch.zeros(len(edges), dtype=torch.bool).to(device)
            edges_3spe_tensor = torch.tensor(edges_3spe.values,dtype=torch.long).to(device)
            for i, edge in enumerate(edges):
                if edge in edges_3spe_tensor:
                    is_in_3spe[i] = True

            regression_loss = torch.where(is_in_3spe, regression_loss * 10, regression_loss)
...
if __name__ == "__main__":
    ...
    args.add_argument("--add_spe3", type=str, default='zebrafish') # None | 'fly' 'zebrafish'
    args.add_argument(
        "--test",
        action="store_true",
        default=False,
    ...
    if config.dg_loss_type == 'graph_mixup':
        file_name = (
            ...
            f"{species_name}/mixupalpha{config.mixup_alpha}_topratio{config.top_ratio}/fold_{config.fold}_{config.epoch}epoch_addspe3_{config.add_spe3}"
        )
    else:
        file_name = (
            base / f"{config.model}/{config.dg_loss_type}/"
    ...
    edges_3spe = get_3th_species(spe=config.add_spe3)
